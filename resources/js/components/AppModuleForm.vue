<template>
    <form @submit.prevent="action" @keydown="form.onKeydown($event)" class="form-horizontal" >
        ...
        <div class="control-group">
            <label class="control-label" for="description">İzin Ön Ek</label>
            <div class="controls">
                <input v-model="form.permission_prefix" type="text" name="permission_prefix" id="permission_prefix">
                <div class="alert alert-danger" v-if="form.errors.has('permission_prefix')">
                    {{ form.errors.get('permission_prefix') }}
                </div>
            </div>
        </div>
        <div class="control-group">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async store() {
                const response = await this.form.post('/api/app_modules');
                this.actionStatusCallback(response.data.data);
            },
            async update() {
                const response = await this.form.put('/api/app_modules/'+this.dataId);
                this.actionStatusCallback(response.data.data);
            },
            async fetch() {
                const response = await window.axios.get('/api/app_modules/'+this.dataId);
